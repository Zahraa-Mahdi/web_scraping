...
def recent_articles():
    ...
    result = list(
        collection.find({}, {"title": 1, "publication_date": 1, "_id": 0}).sort("publication_date", -1).limit(10))
    return jsonify(result)
...
# Route for getting the shortest article
...
def articles_last_X_hours(hours):
    ...
    pipeline = [
        {"$match": {"publication_date": {"$gte": cutoff_date.isoformat()}}},
        {"$project": {"title": 1, "publication_date": 1}}
    ]
    result = list(collection.aggregate(pipeline))
    return jsonify(result)
...
def most_updated_articles():
    pipeline = [
        {"$project": {"title": 1, "update_count": {"$size": {"$split": ["$last_updated_date", "T"]}}}},
        {"$sort": {"update_count": -1}},
        {"$limit": 10}
    ]
    result = list(collection.aggregate(pipeline))
